"""
Run alignment (Phase 1) training on Modal.
...
Usage:
    # Phase A alignment with Qwen3, controller off
    modal run --detach scripts/modal_train_alignment.py --backbone qwen3 --controller off

    # Phase B alignment with B1 (fertility-only) controller
    modal run --detach scripts/modal_train_alignment.py --backbone qwen3 --controller b1

    # Tiny Aya regression
    modal run --detach scripts/modal_train_alignment.py --backbone tiny_aya --controller off

    # Smoke test (batch_size=4, 50 steps)
    modal run scripts/modal_train_alignment.py --backbone qwen3 --controller off --debug-steps 50

    # Override GPU type
    MODAL_GPU=A100-80GB modal run --detach scripts/modal_train_alignment.py --backbone qwen3 --controller off

    # Resume
    modal run --detach scripts/modal_train_alignment.py --backbone qwen3 --controller off --resume-run-id <id>
"""
...
def build_overrides(
    ...
    """Build the Hydra override list for an alignment run."""
    overrides = [
        f"backbone={backbone}",
...
def train(
    ...
    print("=" * 60)
    print("Alignment training on Modal")
    print("=" * 60)
    print(f"GPU: {GPU}")
